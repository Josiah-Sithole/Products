...
to that user, else we say 'Please log in.' More about the tenary operator here: 
https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Conditional_Operator */
function Header(props) {	
  return (<header className="App-header">
	             <div className='Header'>
						<h1>{props.loggedIn ? `Hello, ${props.name}` : 'Please log in'}</h1>
						<h6>Welcome to Designer Motion. This is the place where shoe designs are created and with great success</h6>
						<h6>SignIn to checkout more designs that we have in our back yard store.</h6> 
						<hr/>
						<button className="btn btn-outline-warning btn-lg">{props.button}</button> 
				 </div>	            
	    </header>);
}
